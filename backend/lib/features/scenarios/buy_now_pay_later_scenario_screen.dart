...
class _BuyNowPayLaterScenarioScreenState extends State<BuyNowPayLaterScenarioScreen>
  ...
  StringBuffer analysis = StringBuffer("Here's a breakdown of your choices 🔍 \n\n");

  int bnplCount = 0;
  int positiveChoices = 0;
  int negativeChoices = 0;
  int totalKChange = 0;

  // Iterate through all user choices to provide comprehensive feedback
  for (int i = 0; i < _userChoices.length; i++) {
    ScenarioChoice choice = _userChoices[i];
    // Identify the step based on the index
    // Assuming choices are made in order corresponding to scenario steps
    ScenarioStep step = _scenario.steps[i];
    String stepName = step.npcName;

    // Provide feedback based on step and choice
    if (stepName == "Viktor") {
      // Concert step
    ...
    } else if (stepName == "The Voices" && step.choices.length > 1) {
      // Phone decision step
    ...
    // Add more steps here if future scenario steps are added
  }
